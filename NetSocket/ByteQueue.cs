...
namespace JLM.NetSocket
{
    public class ByteQueue
    {
        ...
        public bool IsLegalPacket
        {
            get { return m_Size >= 4 + GetPacketLength(); }
        }
        ...
        public int GetPacketLength()
        {
            if (m_Size >= 4)
            {
                var size = (int) (m_Buffer[(m_Head)%m_Buffer.Length] | m_Buffer[(m_Head + 1)%m_Buffer.Length] << 8
                                  | m_Buffer[(m_Head + 2)%m_Buffer.Length] << 16 | m_Buffer[(m_Head + 3)%m_Buffer.Length] << 24);
                return size;
            }
