...
namespace TaleofMonsters.DataType.Cards.Weapons
{
    internal sealed class WeaponCard : Card
    {
        ...
        public override void DrawOnStateBar(Graphics g)
        {
            ...
            if (weapon.WeaponConfig.SkillId > 0)
            {
                g.DrawImage(HSIcons.GetIconsByEName("abl9"), adder.Next, 0, 20, 20);
                g.DrawString(ConfigData.GetSkillConfig(weapon.WeaponConfig.SkillId).Name, fontsong, sg, adder.Now+22, 4);
            }
            fontsong.Dispose();
            sb.Dispose();
            sg.Dispose();
