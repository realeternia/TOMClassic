            ...
            if (tar != -1)
            {
                ...
                foreach (DropCardConfig dropCardConfig in ConfigData.DropCardDict.Values)
                {
                    if (dropCardConfig.Pid == people[tar])
                    {
                        ...
                        {
                            var cardData = dropCardConfig.CardId[i];
                            var name = CardConfigManager.GetCardConfig(cardData.Id).Name;
                            string cardstr = string.Format("{0}{1}%", name, cardData.Value);
                            nlClickLabel1.AddLabel(cardstr, new DeckCard(cardData.Id, 1, 0));
                        }
                    }
                }
                Invalidate(new Rectangle(65, 35, cardWidth * xCount + 200, 630));
